			...
			if (xDir)
				if (yDir)
					switch (del.SortCol) {
						...
						case 1: return 0;
						case 2: order = comparer.Compare((lx.Tag as DirectoryInfo).LastWriteTime, (ly.Tag as DirectoryInfo).LastWriteTime);
								return del.SortOrder > 0 ? order : -order;
						default: return 0;
					}
				else {
				...
				if (yDir)
					...
					switch (del.SortCol) {
						case 0: return del.SortOrder > 0 ? comparer.Compare(sx, sy) : comparer.Compare(sy, sx);
						case 1: order = comparer.Compare((lx.Tag as FileInfo).Length, (ly.Tag as FileInfo).Length);
								return del.SortOrder > 0 ? order : -order;
						case 2: order = comparer.Compare((lx.Tag as FileInfo).LastWriteTime, (ly.Tag as FileInfo).LastWriteTime);
								return del.SortOrder > 0 ? order : -order;
						default: return 0;
					}
		}
