...
class RitchisTexteditor : Form {
	...
	ToolStripButton tsbNew, tsbOpen, tsbSave, tsbUndo, tsbRedo, tsbCut, tsbCopy, tsbPaste, tsbFind;
	...
	public RitchisTexteditor() {
		...
		// Bei Symbolleisten ist ein "Zeilenumbruch" bei horizontaler Platznot unüblich:
		//toolStripStandard.LayoutStyle = ToolStripLayoutStyle.Flow;
		...
		tsbUndo.Image = bmp;
		tsbUndo.Click += new System.EventHandler(MitUnRedoOnClick);
		...
		tsbRedo.Image = bmp;
		tsbRedo.Click += new System.EventHandler(MitUnRedoOnClick);
		...
		tsbCut.Image = bmp;
		tsbCut.Click += new System.EventHandler(MitCutOnClick);
		...
		tsbCopy.Image = bmp;
		tsbCopy.Click += new System.EventHandler(MitCopyOnClick);
		...
		tsbPaste.Image = bmp;
		tsbPaste.Click += new System.EventHandler(MitPasteOnClick);
	...
	protected void MitUnRedoOnClick(object sender, EventArgs e) {
		if (sender == mitUndo || sender == cmitUndo || sender == tsbUndo)
			editor.Undo();
		else
			editor.Redo();
	}
	...
	// Enabled-Aktualisierung für die Bearbeiten-Menüitems
	...
	protected void RichTextBoxOnTextChanged(object sender, EventArgs ea) {
		...
		// ToolStripButton tsbUndo umschalten
		if (editor.CanUndo) {
		...
		// ToolStripButton tsbRedo umschalten
		if (editor.CanRedo) {
