...
class RitchisTexteditor : Form {
	...
	string strUserKey, strWordWrap, strScrollBars, strZoomFactor, strBackColor,
		strClientSizeWidth, strClientSizeHeight, strLocationX,strLocationY;
	...
	public RitchisTexteditor() {
		...
		strClientSizeWidth = "ClientSizeWidth";
		strClientSizeHeight = "ClientSizeHeight";
	...
	void FormOnLoad(object sender, EventArgs e) {
		...
		if (userKey != null) {
			try {
				...
				this.ClientSize = new Size((int)userKey.GetValue(strClientSizeWidth), (int)userKey.GetValue(strClientSizeHeight));
				this.Location = new Point((int)userKey.GetValue(strLocationX), (int)userKey.GetValue(strLocationY));
			} catch {
	...
	void FormOnClosing(object sender, CancelEventArgs cea) {
		if (UnsavedText())
			...
			try {
				...
				userKey.SetValue(strClientSizeWidth, this.ClientSize.Width);
				userKey.SetValue(strClientSizeHeight, this.ClientSize.Height);
				userKey.SetValue(strLocationX, this.Location.X);
				userKey.SetValue(strLocationY, this.Location.Y);
			} catch {
