// Ritchis Texteditor mit Symbolleiste, erweitert
using System;
...
class RitchisTexteditor : Form {
	...
	ToolBarButton tbbCut, tbbCopy, tbbPaste, tbbFind, tbbUndo;
	...
	public RitchisTexteditor() {
		...
		string[] tips = {"Neu", "Öffnen", "Speichern", "Suchen", "Ausschneiden",
			"Kopieren", "Einfügen", "Rückgängig"};
		MenuItem[] tags = { mitNew, mitOpen, mitSave, mitFind, mitCut,
			mitCopy, mitPaste, mitUndo};
		for (int i = 0; i < 8; i++) {
		...
		tbbCopy = tb.Buttons[5];
		tbbCopy.Enabled = false;
		tbbPaste = tb.Buttons[6];
		tbbPaste.Enabled = Clipboard.ContainsText();
		tbbUndo = tb.Buttons[7];
